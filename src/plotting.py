...
def plot_level_set_results(bench_function, results):
    ...
    for key in results.keys():
        ...
        if x_list ==0:
            continue
        plt.scatter(x_list[0],x_list[1], s=20, label = key, zorder=1, color = cl, marker=mk)
    plt.xlim(X_domain)
    plt.ylim(Y_domain) 
    plt.tight_layout()
    plt.legend()
...
def plot_function_values(bench_function, results, timeplot = True, add_caption = None):
    ...
    for key in results.keys():
        ...
        try:
            if fvals ==0:
                continue
            plt.plot(fvals, cl , label=key, linewidth=linewidth, markersize=10, marker = mk, markevery= int(np.floor(len(fvals)/5)))
        except:
            plt.plot(fvals, cl , label=key, linewidth=linewidth, markersize=10, marker = mk)
    ...
    if timeplot:
        ...
        for key in results.keys():
            ...
            if key == "Newton":
                continue
            plt.plot(time*np.arange(len(fvals)), fvals, cl , label=key, linewidth=linewidth, markersize=10, marker = mk, markevery= int(np.floor(len(fvals)/5)))
        plt.ylabel('function value')
        plt.xlabel('time')
        plt.yscale('log')
        plt.legend()
