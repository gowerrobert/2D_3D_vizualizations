...
class AckleyN3:
    name = "Ackley N. 3"
    latex_formula = (
        r"f(x, y) = -200exp(-0.2\sqrt{x^2 + y^2}) + 5exp(cos(3x) + sin(3y))"
    )
    latex_formula_dimension = r"d=2"
    latex_formula_input_domain = r"x \in [-32, 32], y \in [-32, 32]"
...
class BohachevskyN3:
    ...
    latex_formula = (
        r"f(x, y)=x^2 + 2y^2 -0.3cos(3\pi x + 4\pi y)cos(4\pi y)+0.3"
    )
    latex_formula_dimension = r"d=2"
...
class DeJongN5:
    ...
    latex_formula_dimension = r"d=2"
    latex_formula_input_domain = (
        r"x \in [-65.536, 65.536], y \in [-65.536, 65.536]"
    )
...
class DixonPrice:
    name = "Dixon Price"
    latex_formula = (
        r"f(\mathbf x) = (x_1 - 1)^2 + \sum_{i=2}^d i(2x_i^2 - x_{i-1})^2"
    )
    latex_formula_dimension = r"d \in \mathbb{N}_{+}^{*}"
    latex_formula_input_domain = (
    ...
    def get_global_minimum(self, d):
        X = np.array(
            [2 ** ((-(2 ** (i)) - 2) / 2**i) for i in range(1, d + 1)]
        )
        return (X, self(X))
...
class GramacyLee:
    ...
    latex_formula_global_minimum = (
        r"f(0.548563444114526) \approx -0.869011134989500"
    )
    continuous = True
    convex = False
    separable = False
...
class Keane:
    ...
    def __call__(self, X):
        x, y = X
        res = -(np.sin(x - y) ** 2 * np.sin(x + y) ** 2) / np.sqrt(
            x**2 + y**2
        )
        return res
...
class Langermann:
    ...
    def __init__(self, d, m=None, c=None, A=None):
        ...
        self.A = (
            A
            if A is not None
            else np.array([[3, 5], [5, 2], [2, 1], [1, 4], [7, 9]])
        )
...
class LevyN13:
    ...
    def __init__(self, d):
        self.d = d
        self.input_domain = np.array([[0.0, 2.0], [0.0, 2.0]])
...
class Michalewicz:
    ...
    latex_formula_global_minimum = (
        r"\text{at $d=2$, }f(2.20, 1.57) \approx -1.8013"
    )
    continuous = True
    convex = False
    separable = True
...
class Ridge:
    name = "Ridge"
    latex_formula = (
        r"f(\mathbf{x})=x_1 + \beta\left(\sum_{i=2}^{d}x_i^2\right)^\alpha"
    )
    latex_formula_dimension = r"d \in \mathbb{N}_{+}^{*}"
    latex_formula_input_domain = (
...
class Rosenbrock:
    ...
    def __init__(self, d, a=1, b=100):
        ...
        self.input_domain = np.array(
            [[-15, 3], [-5, 200]]
        )  # np.array([[-5, 10] for _ in range(d)])
        self.a = a
        self.b = b
...
class Schwefel:
    name = "Schwefel"
    latex_formula = (
        r"f(\mathbf{x})=418.9829d -{\sum_{i=1}^{d} x_i sin(\sqrt{|x_i|})}"
    )
    latex_formula_dimension = r"d \in \mathbb{N}_{+}^{*}"
    latex_formula_input_domain = (
        r"x_i \in [-500, 500], \forall i \in \llbracket 1, d\rrbracket"
...
class Schwefel2_21:
    name = "Schwefel 2.21"
    latex_formula = (
        r"f(\mathbf{x})=\max_{i \in \llbracket 1, d\rrbracket}|x_i| "
    )
    latex_formula_dimension = r"d \in \mathbb{N}_{+}^{*}"
    latex_formula_input_domain = (
        r"x_i \in [-100, 100], \forall i \in \llbracket 1, d\rrbracket"
...
class Shubert:
    ...
    def __call__(self, X):
        ...
        for i in range(0, d):
            res = np.prod(
                np.sum(
                    [i * np.cos((j + 1) * X[i] + j) for j in range(1, 5 + 1)]
                )
            )
        return res
...
class ShubertN3:
    name = "Shubert N. 3"
    latex_formula = (
        r"f(\mathbf{x})=\sum_{i=1}^{d}{\sum_{j=1}^5{j sin((j+1)x_i+j)}}"
    )
    latex_formula_dimension = r"d \in \mathbb{N}_{+}^{*}"
    latex_formula_input_domain = (
        r"x_i \in [-10, 10], \forall i \in \llbracket 1, d\rrbracket"
    ...
    def __call__(self, X):
        d = X.shape[0]
        res = np.sum(
            np.sum([j * np.sin((j + 1) * X + j) for j in range(1, 5 + 1)])
        )
        return res
...
class ShubertN4:
    name = "Shubert N. 4"
    latex_formula = (
        r"f(\mathbf{x})=\sum_{i=1}^{d}{\sum_{j=1}^5{j cos((j+1)x_i+j)}}"
    )
    latex_formula_dimension = r"d \in \mathbb{N}_{+}^{*}"
    latex_formula_input_domain = (
        r"x_i \in [-10, 10], \forall i \in \llbracket 1, d\rrbracket"
    ...
    def __call__(self, X):
        d = X.shape[0]
        res = np.sum(
            np.sum([j * np.cos((j + 1) * X + j) for j in range(1, 5 + 1)])
        )
        return res
...
class StyblinskiTank:
    ...
    latex_formula = (
        r"f(\mathbf{x})=\frac{1}{2}\sum_{i=1}^{d} (x_i^4 -16x_i^2+5x_i)"
    )
    latex_formula_dimension = r"d \in \mathbb{N}_{+}^{*}"
    latex_formula_input_domain = (
        r"x_i \in [-5, 5], \forall i \in \llbracket 1, d\rrbracket"
...
class XinSheYangN2:
    ...
    latex_formula = (
        r"f(\mathbf{x})=(\sum_{i=1}^{d}|x_i|)exp(-\sum_{i=1}^{d}sin(x_i^2))"
    )
    latex_formula_dimension = r"d \in \mathbb{N}_{+}^{*}"
    latex_formula_input_domain = (
        r"x_i \in [-2\pi, 2\pi], \forall i \in \llbracket 1, d\rrbracket"
